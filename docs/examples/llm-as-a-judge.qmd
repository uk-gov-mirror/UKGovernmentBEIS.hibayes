...
These examples demonstrate how to analyse ordinal evaluation data with skewed distributions using ordered logistic regression models. Located in `examples/llm-as-a-judge-usecases/`, they showcase custom model implementations and specialised visualisations for ordinal outcomes.

This notebook implements methods from the paper: [**Quantifying Biases in LLM-as-Judge Evals**](https://openreview.net/pdf?id=q33S4QMuay)
...
::: {layout-ncol=2}
![When we add the interaction the model predicts the data better](../figs/q2_model_comparison.png)
...
Check out how we defined a custom hierarchical model where grader effects inherit a group effect e.g. Human A inherits from the Human parameter in examples/llm-as-a-judge-usecases/custom_model.py.

...
- Interactions: categorical × categorical dims, continuous × categorical slopes. Pass `interactions: true` for all pairwise interactions, or a list of pairs (e.g. `interactions: [[model, task]]`) to build interaction dims only for specific categorical pairs — matching the list-of-pairs format used by model configs
...
| `extract_features` | Full GLM feature extraction with categorical/continuous support | `categorical_features`, `continuous_features`, `interactions` (bool or list of pairs), `effect_coding_for_main_effects`, `standardise`, `test` |
